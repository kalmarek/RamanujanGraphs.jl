@testset "Principal Series" begin
    ...
    Borel_cd = RamanujanGraphs.CosetDecomposition(
        coset_reps,
        inv.(coset_reps),
        Borel(SL₂{q}),
    )
...
@testset "Discrete Series" begin
    ...
    @testset "DiscreteRepr" begin
        ...
        @test all(isapprox.(
            0.0,
            [h.indecomposable[β^i] - h.indecomposable[β]^i for i = 1:q^2-1],
            atol = tol,
        ))
        @test all(isapprox.(
            0.0,
            [h.decomposable[i*α] - h.decomposable[α]^i for i = 1:q],
            atol = tol,
        ))

        Id = Matrix{Float64}(
            I,
            RamanujanGraphs.degree(h),
            RamanujanGraphs.degree(h),
        )
        @test all(isapprox.(
            h(SL2q[1], RamanujanGraphs.Unipotent),
            Id,
            atol = tol,
        ))
        ...
        @test all(
            all(isapprox.(h(a * b), h(a) * h(b), atol = tol))
